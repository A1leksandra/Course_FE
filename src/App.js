...
function HTMLLabViewer({ htmlPath }) {
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100%', background: '#0a0e27' }}>
      <div style={{ padding: '1.5rem', background: 'rgba(20, 24, 45, 0.8)', borderBottom: '1px solid rgba(255, 255, 255, 0.08)', backdropFilter: 'blur(10px)' }}>
        <Link to="/" style={{ color: '#7bc6ff', textDecoration: 'none', fontWeight: '600', fontSize: '0.95rem', transition: 'color 0.2s', display: 'inline-block' }} onMouseEnter={(e) => e.target.style.color = '#ff77c6'} onMouseLeave={(e) => e.target.style.color = '#7bc6ff'}>
          ← Back to Labs
        </Link>
      </div>
      <iframe 
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100%', background: '#0a0e27' }}>
      <div style={{ padding: '1.5rem', background: 'rgba(20, 24, 45, 0.8)', borderBottom: '1px solid rgba(255, 255, 255, 0.08)', backdropFilter: 'blur(10px)' }}>
        ...
        {title && <span style={{ marginLeft: '1.5rem', color: '#a0a0b8', fontSize: '0.9rem', fontWeight: '400' }}>{title}</span>}
      </div>
      <Suspense fallback={
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 'calc(100vh - 60px)', background: '#0a0e27' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '1.5rem', marginBottom: '1rem', color: '#ffffff', fontWeight: '600' }}>Loading lab...</div>
            <div style={{ fontSize: '0.9rem', color: '#6b6b8a' }}>Please wait</div>
          </div>
        </div>
      }>
        {children}
